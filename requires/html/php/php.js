...
var fileX = CreateHTTPRequestObject();
fileX.open("GET",filename,true);
fileX.onreadystatechange = filesizeOnStateChange;
fileX.send(null);
}

function filesizeOnStateChange() {
          if(fileX.readyState==0||fileX.readyState==4) {
   var contLen = fileX.getResponseHeader("Content-Length");
   alert(contLen);
   return contLen;
 } else {
   alert('Failed!');
